...
[TestFixture]
public class MeasurementUnitsControllerTests : TestsBase
{
	public const string BASE_URL = "api/v1/MeasurementUnit";

	[SetUp]
	public void SetUp()
	{
		WebAppFactory = CustomWebApplicationFactory<Program>
			.CreateWithInMemoryDb<Program, EdaDbContext>(MeasurementUnitsTestData.SeedMeasurementUnitsData(null));
	}
	...
	public async Task GetWithoutParameters()
	{
		...
		result!.TotalCount
			.Should().Be(MeasurementUnitsTestData.TestData.Count());
	}

	[Test]
	public async Task GetFive_CheckCorrectSequence()
	{
		var client = WebAppFactory!.CreateClient();
		var firstFive = MeasurementUnitsTestData.TestData.Take(5)
			.Select(x => x.Name)
			.ToArray();
	...
	public async Task Get_SkipAll()
	{
		...
		result!.TotalCount
			.Should()
			.Be(15, $"Total count should be 15. See {nameof(MeasurementUnitsTestData.TestData)}");
		result!.Data
			.Should()
			.BeEmpty();
	}
